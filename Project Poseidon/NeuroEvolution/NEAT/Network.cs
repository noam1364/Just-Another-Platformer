...
[Serializable]
public class Network : IComparable<Network>,Project_Poseidon.NeuroEvolution.NeuralNetwork.INetwork
{
    ...
    /// <summary>
    /// Feeds an input to the neural network,and returns it's output
    /// </summary>
    /// <param name="input">The input to feed to the network</param>
    /// <returns></returns>
    public float[] getOutput(float[] input)
    {
        float[] output = new float[outputNodes.Count];
        ///set the input inside the network, in the input nodes, and fire them
        ...
        while (copyList.Count != 0)
        {
            ...
            ///for each node, check if it recived values from all it's incomning connections, and is ready to 'fire'
            ...
            {   ///if there are 2 nodes or more,that need each other's values to calculated, the network cannot continue, 
                ///so we will force one of those node to calculate it's value even that it has some missing inConnections values.
            ...
            ///remove all nodes that their values were calculated
